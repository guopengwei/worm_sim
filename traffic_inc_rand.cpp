...
bool Traffic_source::get_next_message(Message & msg) {
 ...
 restart:
    while (trace_file.getline(input_line, MAX_LINE, '\n')) {
        ...
        msg.timestamp += trace_file_loop_cnt * period;
        return true;
    }
	...
	if (packet_to_destination_rate) {
	  ...
	  if (buffer.empty_slots() >= on_period) { // Finite source buffer
	    ...
	    if (param.flow_control) {
	      ...
	      unsigned int acceptable = localPort->get_total_acceptable();
	  ...
	  if (!pkt_generated) 
	    ...
	    if (wait_time <= 0)
	      wait_time = 0;
	    next_generation_time = net_clock.get_clock() + on_period + (drand48()*off_period) + wait_time;
	    // net_clock.get_clock() + on_period + off_period + wait_time;
	    // printf("Next generation time for %d is %d, Wait time: %d \n",id,next_generation_time,wait_time);
